...
## Deploy now (fastest options)

### Option A: Render (recommended)

- `render.yaml` is already included.
- In Render Dashboard: **New +** → **Blueprint** → select your repo.
- Add secrets: `ADMIN_API_KEY`, `ADMIN_PASSWORD`, `WHATSAPP_WEBHOOK_URL`.
- Deploy and open:
  - `https://<your-domain>/web/` (customer widget)
  - `https://<your-domain>/web/admin.html` (admin)

### Option B: Docker (any VPS / cloud run)

```bash
docker build -t table-reservation .
docker run -p 8000:8000   -e ADMIN_API_KEY='change-me-admin-key'   -e ADMIN_USERNAME='admin'   -e ADMIN_PASSWORD='change-me-admin-password'   -e WHATSAPP_WEBHOOK_URL='https://your-whatsapp-relay.example.com/send'   -e WHATSAPP_RECIPIENT='0617461487'   table-reservation
```
